...
/*
 ...
 Conatins no VBS Safe mode.. background proseses or rigerouse temporary folder seraching
...
module vbs
	...
	InputBoxFull, InputBox, NullOnInputCancel,
	CreateShortcutFull, CreateShortcut,
	Speach, SpeachWait,
	GetOS, GetOSNum, GetOSFull,
	Download, DownloadSilent, DownloadComplete, DownloadsActive, DownloadName,
	RunHiddenCommand, RunHiddenBatch,
    %Shortucts legacy (compadability redundant)
	msgBox, msgBoxP,
    %misc
	GetWindowsOpen, Quit
    ...
    var returnNullOnCancel : boolean := false %False is the defult
    %When you cancel an inputbox...
    %        true will Return ""
    %        false will Return defult str
    ...
    %Replaces Spaces with web character code for a space
    function webSafeURL (url : string) : string
	result str_replace (" ", "%20", url)
    end webSafeURL

    %Enables \n suport for character retuns
    fcn characterReturn (str : string) : string
	result str_replace ("\n", "\"&chr(13)&\"", str)
    end characterReturn
    ...
    %alows VBS  a moment of time to finish before termination from QUIT call
    fcn QuitTimeOut (var timmer : int) : boolean
	if QUIT then
	...
	vbsID += 1
	var QuitTimmer : int := 800 %just gives vbs a bit more time in milla sec to return when QUIT is called
	loop
	    delay (Rand.Int (5, 80))
	    ...
	    if QuitTimeOut (QuitTimmer) then % Terminate a moment after QUIT call
		result vbsID     %just leave with current count
	    end if
	end loop
    end nextVBSID
	...
	var QuitTimmer : int := 1000   %just gives vbs a bit more time in milla sec to return when QUIT is called
	%Exicutes the VBS File
	...
	loop
	    ...
	    % Terminate a moment after QUIT call
	    if QuitTimeOut (QuitTimmer) then
		exit
	    else
		delay (100)
	    end if
    ...
    function MessageBox (Message : string, buttons : int, icon : string (1), DButton : int, Title : string) : string
	...
	%Filters
	const message : string := characterReturn (exitQuotes (Message))
	const title : string := exitQuotes (Title)
	...
	put : fileNo, "answer=MsgBox(\"" + message + "\"," + intstr (boxType) + ",\"" + exitQuotes (title) + "\")"
    ...
    %short form
    function msgBox (message : string, buttons : int, icon : string (1), DButton : int, title : string) : string
    ...
    %message box process
    process msgBoxProcess (message : string, buttons : int, icon : string (1), DButton : int, title : string, actionProc : procedure p (address : string))
	var answer : string := MessageBox (message, buttons, icon, DButton, title)
	actionProc (answer)
    end msgBoxProcess

    %launch message box process
    procedure MsgBoxP (message : string, buttons : int, icon : string (1), DButton : int, title : string, actionProc : procedure p (address : string))
	delay (150)     %prevents crashes  from spam
	fork msgBoxProcess (message, buttons, icon, DButton, title, actionProc)
    end MsgBoxP

    %Legacy compadiblity shortcut
    procedure msgBoxP (message : string, buttons : int, icon : string (1), DButton : int, title : string, actionProc : procedure p (address : string))
	MsgBoxP (message, buttons, icon, DButton, title, actionProc)
    end msgBoxP



    process NoticeProcess (message : string, icon : string (1), title : string)
	delay (Rand.Int (20, 250))      %prevents crashes  from spam
	var answer : string := MessageBox (message, 0, icon, 1, title)
    end NoticeProcess

    procedure Notice (message : string, icon : string (1), title : string)
	fork NoticeProcess (message, icon, title)
    end Notice

    procedure NoticeWait (message : string, icon : string (1), title : string)
	var answer : string := MessageBox (message, 0, icon, 1, title)
    end NoticeWait
	...
	const message : string := characterReturn (exitQuotes (Message))
	const title : string := exitQuotes (Title)
	const defultText : string := exitQuotes (DefultText)

	%writes the VBS file
	open : fileNo, tmpFolderPath + "\\" + VBSFileName, put
	put : fileNo, " textFeild = InputBox(\"" + message + "\",\"" + title + "\",\"" + defultText + "\")"
	vbsWriteFileS (fileNo, VBSReturnName)
	...
	if returnNullOnCancel then
	    returnNullOnCancel := false
	    result retStr
	else
    ...
    %Temporary enables null string return on cancel click
    procedure NullOnInputCancel
	returnNullOnCancel := true
    end NullOnInputCancel
    ...
    %Downloads a file from the web and returns a query ID for monitoring
    % Calling DownloadComplete() will tell you if its finished downloading


    %---Variables---
    var activeDownloads : int  %Num of download activly downloading
    var downloadQuery : flexible array 1 .. 0 of
	record
	    FileName : string
	    Dest : string
	    Active : boolean
    ...
    %Starts a file download from a web server
    % -1 = falied to start download
    function DownloadEngine (URL : string, dest : string, silent : boolean) : int
	const VBSID : int := nextVBSID ("download")

	const downloadName : string := str_replace ("%20", " ", splitAddress (URL)) %remove and %20 codes
	var saveAsName : string := downloadName
	const url : string := webSafeURL (URL)
	var destPath : string
	loop
	    destPath := dest + "/" + saveAsName
	    if File.Exists (destPath) then
		const lastSaveName : string := saveAsName
		%Create File Diolog
		NullOnInputCancel () %Temp enbles null return mode
		saveAsName := Str.Trim (InputBoxFull ("The file '" + saveAsName + "' already exists. Please re-name to avoid overwriting. ", "SaveAs", saveAsName))
		if saveAsName = lastSaveName then
		    %Overwite??
		    if vbs.MessageBox ("Are you sure you want to overwrite the file '" + downloadName + "' with the new download?", 4, "?", 1, "Overwrite?") = "Yes" then
			exit %Contiue since you want to overwirte
		    end if
		end if
	    else
		exit %continue since there no conflict
	    end if

	    if saveAsName = "" then
		NoticeWait ("The file '" + downloadName + "' was not downloaded.", "i", "Download Aborted")
		result - 1
	    end if
	end loop
	...
	%----Log info to Query----
	const queryID : int := upper (downloadQuery) + 1
	new downloadQuery, queryID
	downloadQuery (queryID).FileName := saveAsName
	downloadQuery (queryID).Dest := destPath
	downloadQuery (queryID).Active := true
	%downloadQuery (queryID).StatusCode := 0

	%----Create and start the downloads script----
	var fileNo : int
	...
	if not silent then
	    put : fileNo, " msgbox\"" + downloadName + " susessfully downloaded.\",0,\"Download SUCCESSFULL\""
	    put : fileNo, "else"
	    put : fileNo, " msgbox\"" + downloadName +
		" was not susessfully downloaded.\" & chr(13) & chr(13) & \"Status: \" & objXMLHTTP.Status  & \" \" & objXMLHTTP.StatusText & \".\" ,vbCritical,\"Download FAILED\""
	end if
	put : fileNo, "End if"
    ...
    fcn Download (url : string, dest : string) : int
	result DownloadEngine (url, dest, false)
    end Download
    fcn DownloadSilent (url : string, dest : string) : int
	result DownloadEngine (url, dest, true)
    end DownloadSilent

    function DownloadComplete (queryID : int) : boolean
	if not downloadQuery (queryID).Active then
	    result true
	end if
	%Download complete when downloaded File Exists
	downloadQuery (queryID).Active := not File.Exists (downloadQuery (queryID).Dest)
	result not downloadQuery (queryID).Active
    end DownloadComplete

    %Retuns the number of download running in the background
    fcn DownloadsActive () : int
	activeDownloads := 0
	for id : 1 .. upper (downloadQuery)
	    if downloadQuery (id).Active then
		activeDownloads += 1
	    end if
	end for
    ...
    %Returns the finnal destination file name of a download
    fcn DownloadName (queryID : int) : string
	result downloadQuery (queryID).FileName
    end DownloadName
    ...
    procedure CreateShortcutFull (targetPath, targetName, destination, Description, arguments, hotKey, iconLocation : string, iconSelection, windowState : int)
	...
	%Filters
	const description : string := characterReturn (exitQuotes (Description))
	...
	put : fileNo, " link.Description = \"" + description + "\" "
	...
	for i : 1 .. 2500 by 50     %2.5 sec timeout
	    %Wait for vbs to create shortcut
	    exit when File.Exists (targetPath + "/" + targetName + ".lnk")
	    delay (50)
	end for
    ...
    procedure CreateShortcut (targetPath, targetName, destination, description : string)
    ...
    procedure CreateWebLink (url, destination : string)
	%CreateShortcutFull ()
    end CreateWebLink
