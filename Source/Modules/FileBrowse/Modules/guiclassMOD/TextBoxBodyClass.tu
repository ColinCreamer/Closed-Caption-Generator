...
class TextBoxBodyClass
    ...
    %export selectLine /*, SetTextBoxColour*/ %(RCC)
    ...
    var userActionProc : procedure actionProc (line : int, chosen : boolean)
    ...
    var textBoxColor : int %RCC)
    var textColor : int %RCC)
    var selctionBoxColor : int := blue


    % procedure SetTextBoxColour (clr : int) %(RCC)
    %    textBoxColor := clr
    %    put "hey",textBoxColor
    % end SetTextBoxColour


    function autoBW (Color : int) : int %RCC
	var OR, OG, OB : real := 0 %Original RGB
	RGB.GetColor (Color, OR, OG, OB)
	if OR + OB + OG < 1.5 then %BW Desision
	    result white
	else
	    result black
	end if
	if OG > 0.7 then %green exception
	    result black
	end if
    end autoBW


    function colorTint (Color : int, Tint : int) : int %RCC
	var OR, OG, OB : real := 0 %Original RGB
	var TR, TG, TB : real := 0 %Tint RGB
	var NR, NG, NB : real := 0 %New RGB
	var colorSlot := 254
	RGB.GetColor (Color, OR, OG, OB)
	RGB.GetColor (Tint, TR, TG, TB)
	...
	% end if
	Draw.FillBox (x1, y1, x2, y2, textBoxColor) %(RCC)
	...
	for cnt : 1 .. screenLines
		...
		Draw.FillBox (x1, textY - lineDescent, x2,
		    textY + lineHeight - lineDescent, selctionBoxColor)
		Font.Draw (linePtr -> text, x1 - xOffset + 4, textY, textFontID,
		    autoBW (selctionBoxColor))
	    else
		Font.Draw (linePtr -> text, x1 - xOffset + 4, textY, textFontID,
		    textColor)
	    end if
    ...
    body procedure InitializeChoice (x, y, width, height, newBorder,
	    fontID, TBColor, tColor, sTint : int, newActionProc : procedure x (line : int, chosen : boolean))
	    textBoxColor := TBColor
	    textColor := tColor
	    selctionBoxColor := colorTint (TBColor, sTint)
	...
	body procedure InitializeFull (x, y, width, height : int,
	    ...
	    userActionProc := TextBoxModule.DoNothing2
	    selectable := false
	...
	%Colin Creamer
	%Oct 20 2010
	body procedure SetSelectedLine (lineNumber : int)
	    var scrollY : int := vScroll -> GetSliderValue
	    selectedLine := lineNumber
	    %jumps down a page
	    if selectedLine >= scrollY + screenLines then
		vScroll -> SetSliderValue (selectedLine)
	    end if
	    %jumps up a page
	    if selectedLine < scrollY then
		vScroll -> SetSliderValue (selectedLine - screenLines)
	    end if
	    DrawText %Refresh
	end SetSelectedLine
	...
	body function ConsiderButtonDown (mouseX, mouseY : int) : boolean
		...
		userActionProc (line, true)
		result true
	    else
		userActionProc (line, false)
	    end if
	    selectedLine := line
	    DrawText
	    result true
