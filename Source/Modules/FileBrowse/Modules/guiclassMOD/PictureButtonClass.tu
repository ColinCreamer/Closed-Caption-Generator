...
class PictureButtonClass
    ...
    %Colin Creamer
function colorTint (Color : int, Tint : int) : int
	...
	Pic.Draw (buttonPic, picX + 1, picY - 1, drawMode)
	if not enabled then
	    for i : x1 + riserSize + 1 .. x2 - riserSize by 2
		for j : y1 + riserSize + 1 .. y2 - riserSize by 2
		    Draw.Dot (i, j, colorTint(whatdotcolor(i,j),210))
		    Draw.Dot (i + 1, j + 1, colorTint(whatdotcolor(i+1,j+1),210))
		end for
	    end for
	end if
	View.ClipOff
