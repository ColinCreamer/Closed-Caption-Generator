...
class GenericLabelledWidgetClass
    ...
    %Colin Creamer
    var pressedOffset : boolean := false %offsets a lable to give the ilution of depth whhen a button is pressed
    ...
    %This mod makes the generic gui buton text apear to 
    %move with the buton when it is pressed in and out.
    
	var offset : int := 0       %colin creamer
	if pressedOffset then       %colin creamer
	    offset := 1             %colin creamer
	    pressedOffset := false  %colin creamer
	end if                      %colin creamer
	var x : int := x + labelX + offset
	var y : int := y + labelY - offset
	if not initialized then
	    AssertFailed ("Labelled widget not yet initialized")
	else
	    Font.Draw (labelText, x, y, labelFontID, clr)
